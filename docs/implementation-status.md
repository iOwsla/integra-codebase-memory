...
| 10 Documentation/release | IN PROGRESS | README, ADRs, guides, issue template, changelog, limitations; hosted Linux prerelease CI verified; project-scoped Codex setup, MCP instructions and acceptance checklist added; stable-release hardening gates remain |
...
Final verification results are recorded in `docs/verification.md`. Hosted Linux CI and the release workflow passed for v0.1.0-alpha.9; each subsequent release must pass its own gates.
