...
See `docs/limitations.md` for each incomplete requirement, current implementation and recommended next action. Prioritize scoped SQL query pagination and memory search filters before declaring production release readiness. Optional framework plugins, embeddings, cross-project memory and a daemon are intentionally not implemented.
