...
Index query metadata and bounded results share one read-only REPEATABLE READ transaction. Full snapshots for index reconciliation also use REPEATABLE READ. Graph publication is transactional across the selected project, stronger than file atomicity. A dedicated connection holds a project advisory lock from pre-scan state read through publication. Memory is stored independently and survives code index cleanup.
