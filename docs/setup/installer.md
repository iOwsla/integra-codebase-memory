...
```sh
curl -fsSL https://raw.githubusercontent.com/iOwsla/integra-codebase-memory/v0.1.0-alpha.24/bootstrap.sh | sh -s -- --project /absolute/target-project --client both --write
```
...
Default runtime: `$XDG_DATA_HOME/integra-code-memory/releases/v0.1.0-alpha.24`,
or `$HOME/.local/share/integra-code-memory/releases/v0.1.0-alpha.24`. Override with
...
```powershell
& ([scriptblock]::Create((Invoke-WebRequest -UseBasicParsing 'https://raw.githubusercontent.com/iOwsla/integra-codebase-memory/v0.1.0-alpha.24/bootstrap.ps1').Content)) -Project 'C:\Projects\My App' -Client both -Write
```

Choose `codex`, `claude` or `both`. Omit `-Write` for a no-write preview.
The runtime defaults to `%LOCALAPPDATA%\integra-code-memory\releases\v0.1.0-alpha.24`;
...
```powershell
Invoke-WebRequest -UseBasicParsing 'https://raw.githubusercontent.com/iOwsla/integra-codebase-memory/v0.1.0-alpha.24/bootstrap.ps1' -OutFile bootstrap.ps1
