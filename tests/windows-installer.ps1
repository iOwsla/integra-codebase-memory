...
try {
    ...
    if ($env:TEST_PUBLISHED_BOOTSTRAP -eq 'true') {
        ...
        Invoke-WebRequest -UseBasicParsing -Uri 'https://raw.githubusercontent.com/iOwsla/integra-codebase-memory/v0.1.0-alpha.17/bootstrap.ps1' -OutFile $downloadedScript
    ...
    function git {
        ...
        elseif ($args[2] -eq 'describe') { 'v0.1.0-alpha.17' }
    }
    function bun {
        $global:LASTEXITCODE = 0
