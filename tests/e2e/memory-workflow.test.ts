...
const fs=require('fs');let input='';process.stdin.on('data',d=>input+=d);process.stdin.on('end',()=>{const data=JSON.parse(input.trim().split('\\n').at(-1));const output={candidates:[{id:'c1',claim:'Validate refund quantities before saving.',classification:'REQUIREMENT',evidenceIds:[data.evidenceSegments[0].evidenceId]}]};fs.writeFileSync(process.argv[process.argv.indexOf('--output-last-message')+1],JSON.stringify(output));console.log(JSON.stringify({type:'turn.completed',usage:{input_tokens:20,output_tokens:20}}));});`;
